#C5.0 Decision Tree - For only Classification - Video 51
...
data = read.csv(file = "C:/Kishor/DoNotTouch/Insofe_Videos/8th Set/UniversalBank.csv",col.names = attr, header = TRUE)

#Check the correlation between the attributes and remove the unwanted columns, this is not important in Decision tree
cor(data)
...
DCT = C5.0(personal ~ .,data = train_data,rules = TRUE)   #This will create a Decision tree model (personal is the traget)
summary(DCT)
C5imp(object = DCT,pct = TRUE)   #This is to check the variable importance in decision tree, pct -> 0 to 100
