...
data = read.csv(file = "C:/Kishor/DoNotTouch/Insofe_Videos/8th Set/UnivBank.csv",header = TRUE,col.names = attr)
colnames(data)
str(data)
...
printcp(cart_model)    #Check the complexity parameter for each iteration, and 
                       #the iteration/cp with least xerror is used for model building
...
#train.y is used to get nmse and nmae -> which is the relative measure to understand the performance of the model
#with the baseline model And mape is also used to measure the performance with the baseline model

#Select the appropriate complexity parameter to avoid over fitting of training data

printcp(cart_model)  

#We need to select one with the least xerror
bestcp = cart_model$cptable[which.min(cart_model$cptable[,'xerror']),"CP"]  #From CP table select the CP cloumn with min xerror column value
